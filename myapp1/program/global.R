...
if(!file.exists(paste0(data_dir, "ships.csv"))) {
    ...
    g_ships <- vroom(paste0(data_dir, "ships_04112020.zip"), delim = ",", show_col_types = FALSE)
    vroom_write(x = g_ships, file = paste0(data_dir, "ships.csv"), delim = ",")
} else {
    # Read the csv file.
    g_ships <- vroom(paste0(data_dir, "ships.csv"), delim = ",", show_col_types = FALSE)
}
